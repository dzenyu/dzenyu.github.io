...
## Why CDC Instead of Application-Level Dual Writes?

The central synchronization decision is deliberately simple:

**RDBMS → CDC → Redis**

The Provisioning Service does not write the same location record independently to both the RDBMS and Redis.
...
This creates a partial-failure problem. The RDBMS write can succeed while the Redis write fails, or vice versa. The application then has to decide how to reconcile two systems that may disagree.

It also couples provisioning latency to the availability and response time of the serving layer.

### Database fallback from the read path
...
This keeps the write path simple but creates a potentially dangerous fallback. A cache miss during a provisioning spike pushes read traffic back onto the same database absorbing the write workload.

For an emergency location service, that coupling is undesirable.
...
The RDBMS remains the single write target and source of truth. CDC provides the asynchronous synchronization mechanism, while Redis provides the read-optimized serving model.

The tradeoff is replication lag. CDC lag therefore becomes a first-class operational metric that must be monitored and validated.
