...
![Unified E911 architecture diagram: the Location Delivery read path (PSAP → Location Service → Redis PIDF-LO data) on the left, the Provisioning write path (User → Provisioning Service → RDBMS) on the right, CDC replicating RDBMS changes into Redis, and the Audit Replication System dequeuing audit data from Redis and persisting it back to the RDBMS](/assets/images/architecting-nextgen-e911-location-delivery-unified-architecture.png)
